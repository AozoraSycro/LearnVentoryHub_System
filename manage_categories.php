...
<head>
    ...
    <style>
        ...
        /* ช่องเลือกหมวดหมู่ และ ค้นหาสินค้า */
        .filter-container {
            display: flex;
...
<h2 style="text-align: center; padding-top: 20px; font-size: 35px; font-weight: bold;">✿ ขายสินค้า ✏️</h2>
    ...
    <form id="filterForm" method="GET" style="display: flex; gap: 10px; align-items: center;">
        <select name="c_ID" id="categorySelect" onchange="document.getElementById('filterForm').submit()">
        ...
        <input type="text" id="searchProduct" name="search" placeholder="🔍 ค้นหาสินค้า..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
    </form>
</div>
                    ...
                    <tbody>
                        <?php while ($row = $products->fetch_assoc()) { ?>
                            <tr>
                                ...
                                <td><?= $row['p_total'] ?></td> <!-- เพิ่มแสดงยอดคงเหลือ -->
                                <td><input type="number" id="qty_<?= $row['p_ID'] ?>" value="0" min="1" max="<?= $row['p_total'] ?>"></td>
    ...
    <script>
        ...
        function addToCart(id, name, price, stock) {
            ...
            if (qty <= 0) {
                alert("กรุณาเลือกจำนวนสินค้ามากกว่า 0 ก่อนเพิ่มลงตะกร้า!");
            return;
    }
            if (qty > stock) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            location.reload(); // รีเฟรชหน้าทันทีเพื่ออัปเดตสต็อก
        },
        error: function() {
